/*!
 * idb v7.1.1 — Promise-based IndexedDB wrapper.
 *
 * Vendored locally for M36 — a PWA whose offline mode is bootstrapped
 * by a CDN script is contradictory. Pinned at 7.1.1 (latest stable as
 * of 2026-05-17).
 *
 * Upstream: https://github.com/jakearchibald/idb
 * NPM:      https://www.npmjs.com/package/idb
 * UMD URL:  https://unpkg.com/idb@7.1.1/build/umd.js
 *
 * Exports (on `window.idb` in browsers):
 *   openDB(name, version, options?)  — open / create / upgrade a database
 *   deleteDB(name, options?)         — delete a database
 *   wrap(idbThing)                   — wrap a raw IDB primitive in the proxy
 *   unwrap(wrapped)                  — unwrap a wrapped value to its raw form
 *
 * Licence: ISC (essentially equivalent to MIT for redistribution).
