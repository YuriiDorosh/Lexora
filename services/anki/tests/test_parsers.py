...
from main import _clean_field, _parse_apkg, _parse_txt, _process_job
...
def _make_apkg(
    notes: list[str], media: dict | None = None, audio_files: dict | None = None
) -> bytes:
...
class TestProcessJob:
    def test_routes_txt(self):
        ...
        entries, audio, errors = _process_job(
            {"file_format": "txt", "file_data": b64, "field_mapping": "{}"}
        )
        assert len(entries) == 2
        assert audio == {}
    ...
    def test_routes_apkg(self):
        ...
        entries, audio, errors = _process_job(
            {"file_format": "apkg", "file_data": b64, "field_mapping": "{}"}
        )
        assert len(entries) == 1
