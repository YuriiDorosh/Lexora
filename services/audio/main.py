...
def _init_whisper():
    ...
    try:
        ...
        _logger.info(
            "Loading faster-whisper model=%s into %s ...", WHISPER_MODEL, WHISPER_MODEL_DIR
        )
        ...
        _logger.error(
            "faster-whisper load failed: %s — transcription will return empty strings", exc
        )
...
def _process_transcription_job(job_id: str, payload: dict) -> dict:
    ...
    _logger.info(
        "STT job: job_id=%s audio_id=%s lang=%s audio_b64_len=%d",
        job_id,
        audio_id,
        language,
        len(audio_b64),
    ...
    _logger.info(
        "STT complete: job_id=%s chars=%d transcript=%r", job_id, len(transcript), transcript[:80]
    )
    return {
        "audio_id": audio_id,
...
def _handle_message(channel, method, _properties, body):
    ...
    _logger.info(
        "Processing job_id=%s on queue=%s | inner_payload_keys=%s",
        job_id,
        routing_key,
        list(payload.keys()),
    )
...
def _consumer_loop():
    ...
    while True:
        try:
            ...
            _logger.info(
                "Audio consumer: connecting to RabbitMQ at %s:%d vhost=%s",
                RABBITMQ_HOST,
                RABBITMQ_PORT,
                RABBITMQ_VHOST,
            )
