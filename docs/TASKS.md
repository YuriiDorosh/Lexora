...
**Scope:** Fully automated helpdesk — every new `helpdesk.ticket` record receives an
instant OdooBot reply generated by a local RAG pipeline (pgvector + llama-cpp-python).
Zero human agents. Zero cloud API calls. CPU-only.
...
- [x] M26-09 · `src/addons/language_helpdesk/__manifest__.py` — depends on `helpdesk`, `mail`. ✅
- [x] M26-10 · `src/addons/language_helpdesk/data/ir_config_parameter.xml` — seeds `language.ai_mentor.url`, `language.ai_mentor.timeout`, `language.ai_mentor.enabled`. ✅
- [x] M26-11 · `src/addons/language_helpdesk/models/helpdesk_ai.py` — `_inherit = 'helpdesk.ticket'`; overrides `create()`; calls `/answer` synchronously; fallback stub on timeout/error; injects reply via `message_post(author=OdooBot)`. ✅
- [x] M26-12 · `Makefile` — `AI_MENTOR_CONTAINER`, `up-ai-mentor`, `down-ai-mentor`, `up-ai-mentor-no-cache`, `logs-ai-mentor` added; ai_mentor wired into `up-dev`, `down-dev`, `ps-dev`, `logs-dev`. ✅
- [x] M26-13 · `env.example` — `AI_LLM_*`, `AI_EMBED_*`, `AI_RAG_*` vars documented with upgrade path (Phi-3 Mini). ✅
- [x] M26-14 · `docs/PLAN.md` — bumped to v1.4; M22–M25 marked ✅ Complete; M26 row added; full M26 spec + updated dependency graph appended. ✅
- [ ] M26-15 · Rebuild Postgres with pgvector: `make down-db && make up-db`; confirm `CREATE EXTENSION vector` succeeds.
- [ ] M26-16 · `make up-ai-mentor-no-cache` — image builds clean (llama-cpp-python compiles or uses manylinux wheel); container starts; `/health` responds.
- [ ] M26-17 · First-boot model download completes; `/health` flips `llm_ready:true` and `embeddings_ready:true`.
- [ ] M26-18 · Seed docs ingested automatically: `ai_mentor_docs` table has 10 rows.
- [ ] M26-19 · Direct `/answer` test: `curl -X POST http://localhost:8005/answer -d '{"ticket_id":1,"subject":"How do I add words?","description":"I want to save vocabulary.","language":"en"}'` → `{"reply":"...","rag_used":true}`.
- [ ] M26-20 · Install `language_helpdesk` addon: `docker exec odoo odoo --config /etc/odoo/odoo.conf -d lexora --init language_helpdesk --stop-after-init`.
- [ ] M26-21 · E2E test: create helpdesk ticket via portal → OdooBot reply appears in chatter within 35 s.
- [ ] M26-22 · Regression: existing modules unaffected by pgvector image swap and new addon.
- [ ] M26-23 · Commit and push `m26_ai_helpdesk`.

#### Architecture notes

**Synchronous HTTP call from Odoo to ai_mentor:** Same pattern as M17 (roleplay).
`requests.post(f"{service_url}/answer", json={...}, timeout=35)` is called inside
`helpdesk.ticket.create()`. Result is decoded with `resp.content.decode("utf-8",
errors="replace")` + `json.loads(raw)` (content-type agnostic, identical to M17).

**Stub reply on any failure:** `helpdesk_ai.py` catches `requests.exceptions.Timeout`
and all other exceptions, logging each. The stub reply acknowledges the ticket and
directs users to self-service resources. Users never see a raw error.
...
(`IF NOT EXISTS`) so re-running init code on a warm DB is harmless.

**Seeding:** `_seed_if_empty()` runs once on first boot (via `_wait_and_seed` daemon
thread). It embeds 10 knowledge documents covering all major Lexora features and inserts
them into `ai_mentor_docs`. Subsequent restarts skip seeding (row count > 0 guard).

**Env-configurable disable:** Set `language.ai_mentor.enabled = 0` in `ir.config_parameter`
to pause auto-reply without uninstalling the addon.
