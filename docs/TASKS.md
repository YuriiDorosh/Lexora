...
**Scope:** Fully automated helpdesk — every new `lexora.ticket` record receives an
instant OdooBot reply generated by a local RAG pipeline (pgvector + llama-cpp-python).
Zero human agents. Zero cloud API calls. CPU-only. Authenticated users see their
full ticket history with AI replies at `/my/tickets`.

**Architecture pivot note:** The original plan referenced `language_helpdesk` (depending
on `helpdesk`) and `odoo_website_helpdesk` (Cybrosys third-party, 160+ files). Both were
abandoned as too bloated and hard to customise. A custom `lexora_helpdesk` module was
built from scratch — self-contained, no third-party deps, `lexora.ticket` model.

#### Sub-steps

- [x] M26-01 · Branch `m26_ai_helpdesk` created from `m25_newtab_dashboard`. ✅
- [x] M26-02 · TASKS.md updated — M25 archived, M26 block started. ✅
- [x] M26-03 · `docker_compose/db/docker-compose.yml` — `postgres:15` → `pgvector/pgvector:pg15` (both postgres + backup services). ✅
- [x] M26-04 · `services/ai_mentor/requirements.txt` — all deps pinned: `llama-cpp-python`, `fastembed`, `psycopg2-binary`, `pgvector`, `huggingface-hub`, `requests`. ✅
- [x] M26-05 · `services/ai_mentor/__init__.py` — created (empty, marks package). ✅
- [x] M26-06 · `services/ai_mentor/main.py` — full implementation: pgvector init + ivfflat index, fastembed ONNX embeddings, llama-cpp GGUF loading with HF auto-download, `_retrieve()` ANN cosine search, `_generate()` LLM synthesis, 10 seed documents, `_seed_if_empty()` daemon thread, FastAPI `/health` + `/answer` + `/ingest` endpoints. ✅
- [x] M26-07 · `docker_compose/ai_mentor/Dockerfile` — `python:3.11-slim` + cmake/build-essential for llama-cpp-python; `HF_HOME` + `FASTEMBED_CACHE_PATH` env vars. ✅
- [x] M26-08 · `docker_compose/ai_mentor/docker-compose.yml` — port 8005:8000, `ai_mentor_models` named volume, pgvector + LLM + embed + RAG env vars. ✅
- [x] M26-09 (superseded) · `language_helpdesk` + `odoo_website_helpdesk` — ABANDONED. Both removed via `git rm -r`. See architecture pivot note above.
- [x] M26-10 · `src/addons/lexora_helpdesk/` — custom self-contained addon created:
  - `models/lexora_ticket.py` — `lexora.ticket` with `mail.thread`/`mail.activity.mixin`, `create()` override calls `/answer`, OdooBot reply via `message_post()` with email-silencing context. ✅
  - `controllers/portal.py` — `GET/POST /support` (public); `CustomerPortal` subclass for `/my/tickets` + `/my/tickets/<id>`. ✅
  - `views/portal_support.xml` — dark gradient hero, two-field form, CSRF token, thanks page. ✅
  - `views/portal_tickets.xml` — portal home widget, ticket list with pager, detail page with chatter. ✅
  - `views/lexora_ticket_views.xml` — backend list/form/search + Helpdesk backend menu. ✅
  - `security/ir.model.access.csv` — user/admin/portal access. ✅
  - `security/record_rules.xml` — users see only own tickets; admins see all. ✅
  - `data/ir_config_parameter.xml` — `lexora.ai_mentor.{url,timeout,enabled}` (noupdate=1). ✅
  - `data/website_menu.xml` — "Support" navbar → `/support`, sequence 90. ✅
- [x] M26-11 · `Makefile` — `AI_MENTOR_CONTAINER`, `up-ai-mentor`, `down-ai-mentor`, `up-ai-mentor-no-cache`, `logs-ai-mentor` added; ai_mentor wired into `up-dev`, `down-dev`, `ps-dev`, `logs-dev`. ✅
...
**Custom `lexora.ticket` model:** No dependency on Odoo's enterprise `helpdesk` module or
any third-party addon. The model inherits `mail.thread` + `mail.activity.mixin` for the
chatter. `create()` is overridden with `@api.model_create_multi`.

**Synchronous HTTP call from Odoo to ai_mentor:** Same pattern as M17 (roleplay).
`requests.post(f"{service_url}/answer", json={...}, timeout=35)` called inside
`_ai_auto_reply()`. Result decoded with `resp.content.decode("utf-8", errors="replace")`
+ `json.loads(raw)` (content-type agnostic, identical to M17).

**Email silencing:** `message_post()` called with context vars
`mail_post_autofollow=False`, `mail_create_nosubscribe=True`, `mail_notify_force_send=False`
to prevent Gmail notifications. OdooBot becomes the author but no email is flushed.

**Portal history:** `TicketCustomerPortal` extends `CustomerPortal` (from `portal` module).
`_prepare_home_portal_values` injects `ticket_count` into the portal home counter.
Row-level security uses `ir.rule` so the SQL domain `[('user_id','=',user.id)]` is applied
automatically — the controller's explicit `.search()` filter is defence-in-depth.

**Config param prefix change:** From `language.ai_mentor.*` (old `language_helpdesk`)
to `lexora.ai_mentor.*` (new `lexora_helpdesk`). Seeded via `noupdate="1"` so existing
values on a live DB are not overwritten on `--update`.

**pgvector image:** `pgvector/pgvector:pg15` is a drop-in for `postgres:15` — same
base, same entrypoint, same volume layout. `CREATE EXTENSION vector` is idempotent.

**Seeding:** `_seed_if_empty()` runs once on first boot (daemon thread). 10 knowledge
documents embedded and inserted into `ai_mentor_docs`. Subsequent restarts skip seeding.
