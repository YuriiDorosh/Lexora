...
**Scope:** Add a second tab to the M36 mobile PWA. Native-app-style
bottom nav switches between **Practice** (the M36 swipe-card flow) and
**Dictionary** (a new read-only browse-all-vocabulary view with
client-side search). The dictionary works fully offline via a new
IndexedDB store; one new Odoo route prefetches the data on Wi-Fi.

**Architectural decisions (locked at S1 — formalised inline since
this milestone is too small for a dedicated ADR):**

| # | Decision | Why |
|---|---|---|
| (a) | New `/lexora_api/offline_vocabulary` route, NOT extending M34's `/lexora_api/my_vocab` | M34's route filters on `pvp_eligible=True` (excludes entries with pending translations) and caps at 1000 ordered by `write_date desc`. Dictionary needs ALL active entries (including pending-translation ones, so the user sees the source word) and reads alphabetically. Two routes with single responsibilities. |
| (b) | IDB schema v1 → v2 via additive `upgrade(db, oldVersion)` callback | Standard IDB migration pattern. Existing M36 users (`oldVersion === 1`) get ONLY the new `vocabulary_cache` store; fresh installs (`oldVersion === 0`) create all three stores in one block. No data migration; the new store starts empty and fills on first online prefetch. |
| (c) | DOM-stable filter (toggle `display:none` rows, do NOT rebuild) | 2000 vocabulary rows can lag if we rebuild on every keystroke. Filter pass = O(n) iteration over a pre-built JS array of `{row, haystack}` pairs; haystack is lowercased word + translations joined for cheap `includes()`. Sub-millisecond on modern phones. |
| (d) | Bump SW `VERSION` to `'lexora-pwa-v2'` | The SW source changed (`LEXORA_API_RE` extended); a bump triggers the M36-S5 user-controlled-update flow for the first time in production. Validates that the banner UX works on real users, not just our manual test. |

#### Sub-steps

**Step M37-S1 — Odoo API + IDB schema v2** ✅

- [x] M37-S1-01 · `portal_pwa.py` — new module constant
  `_OFFLINE_VOCAB_DEFAULT_LIMIT = int(os.environ.get(
  'LEXORA_OFFLINE_VOCAB_LIMIT', '2000'))`.
- [x] M37-S1-02 · New module-level helper
  `_project_vocab_for_user(env, user, limit)`. Reads
  `language.entry` directly (not `language.review`), filter
  `owner_id = uid AND status='active'`; sort by
  `lower(source_text) asc` so the dictionary reads
  alphabetically. Translation bulk-fetch identical to
  `_project_cards_for_user` (first-write-wins per `(entry, lang)`).
- [x] M37-S1-03 · New route
  `GET /lexora_api/offline_vocabulary` (`auth='user'`,
  `type='http'`). Wraps the helper output as
  `{status:'ok', words:[...], generated_at:int(time.time())}`.
  Headers: `Content-Type: application/json; charset=utf-8`,
  `Cache-Control: no-store` (IDB owns offline data on the
  client side; SW never caches user payloads).
- [x] M37-S1-04 · `lexora_db.js` — bump `DB_VERSION` from 1 → 2.
  Add `STORE_VOCAB = 'vocabulary_cache'` constant.
- [x] M37-S1-05 · `init()` upgrade callback — append
  `if (oldVersion < 2) { db.createObjectStore(STORE_VOCAB,
  {keyPath:'id'}); }`. Existing `if (oldVersion < 1)` block
  untouched so v0 → v2 fresh installs create all three stores.
- [x] M37-S1-06 · Two new DB methods on the public API:
  - `replaceVocabulary(words)` — wholesale `clear()` + bulk-put
    in one transaction. Returns `{ok:true, count}` or
    `{ok:false, error, message}` matching the rest of the API.
  - `getVocabulary()` — `getAll()` then alphabetical sort by
    `normalized` (with lowercased-source-text fallback for rows
    that somehow lack `normalized`). Returns `{ok, words}`.
  - `stats()` extended with `vocabCount` field.

**Step M37-S2 — Mobile UI: bottom nav + dictionary panel** ✅

- [x] M37-S2-01 · `portal_practice_mobile.xml` — wrap existing
  `.lx-mp-stage` + `.lx-mp-actions-row` in a new
  `<section id="lx-mp-panel-practice" class="lx-mp-panel">`.
- [x] M37-S2-02 · New `<section id="lx-mp-panel-dictionary"
  class="lx-mp-panel d-none">`:
  - Sticky search input (`<input type="search"
    id="lx-mp-dict-search">`) at top with placeholder
    "Search your vocabulary…".
  - `<ul id="lx-mp-dict-list">` for the rendered rows.
  - Loading state + empty state placeholders.
- [x] M37-S2-03 · `<nav id="lx-mp-bottom-nav">` fixed at the
  bottom with two `<button data-tab="...">` elements. The
  default tab is `practice` (so M36 UX is preserved by default).

**Step M37-S3 — JS state + tab switch + search filter** ✅

- [x] M37-S3-01 · `mobile_practice.js` — new
  `state.activeTab = 'practice'` + `state.dictionaryLoaded = false`
  + module-level `_dictRows = []`.
- [x] M37-S3-02 · `_switchTab(name)`:
  - Toggle `d-none` on `#lx-mp-panel-practice` /
    `#lx-mp-panel-dictionary`.
  - Toggle `lx-mp-tab-active` + `aria-current` on the bottom-nav
    buttons.
  - Hide the M36 `#lx-mp-actions-row` on Dictionary; show on
    Practice. (Forgot / Remembered buttons only make sense in
    review mode.)
  - First switch to Dictionary triggers `_renderDictionary()`.
- [x] M37-S3-03 · `_renderDictionary()`:
  - `await lexora.db.getVocabulary()`.
  - Empty list → show empty-state copy with a hint to add
    vocabulary from the desktop site / browser extension.
  - Non-empty → build `<li>` rows via `_buildDictRow(entry)`;
    store `{row, haystack}` in `_dictRows` for filter.
  - `haystack` = lowercased(word + ' ' + Object.values(translations).join(' ')).
- [x] M37-S3-04 · `_filterDictionary(query)`:
  - Empty query → reset all rows to `display:''`.
  - Non-empty → iterate `_dictRows`, set `display:'none'` for
    rows whose haystack doesn't include the lowercased query.
  - Pure DOM mutation, no rebuild.
- [x] M37-S3-05 · `_prefetchVocabulary()` — best-effort GET to
  `/lexora_api/offline_vocabulary`. On 200, call
  `replaceVocabulary`. If Dictionary tab is currently active,
  re-render. Wire alongside `_prefetchBatch()` in `boot()` and
  in `_setOnline(true)`.

**Step M37-S4 — CSS + SW** ✅

- [x] M37-S4-01 · `mobile_practice.css` — bottom-nav rules:
  fixed position bottom; safe-area-inset-bottom padding;
  2-col flex/grid; icon + label per tab; indigo active state;
  ~64 px tall.
- [x] M37-S4-02 · Panel layout rules — both panels
  `flex: 1 1 auto; min-height: 0; overflow: hidden` so the
  scrollable child works inside the column flex; bottom nav
  reserved space prevents content underlap.
- [x] M37-S4-03 · Dictionary list rules — `.lx-mp-dict-row`
  flex layout (word + lang pill + translations); sticky
  `.lx-mp-dict-search` at top with `backdrop-filter: blur`
  matching the header style; rows separated by
  `border-bottom: 1px solid rgba(255,255,255,0.06)`.
- [x] M37-S4-04 · `sw.js` — bump `VERSION` to `'lexora-pwa-v2'`;
  extend `LEXORA_API_RE` regex to add `offline_vocabulary` as
  the third alternative:
  `/^\/lexora_api\/(?:offline_batch|sync_offline|offline_vocabulary)(?:\?|$)/`.

**Step M37-S5 — Verification** ✅

- [x] M37-S5-01 · Static checks: `node --check sw.js`,
  `node --check lexora_db.js`, `node --check mobile_practice.js`,
  `python3 ast.parse portal_pwa.py`,
  `xml.etree.ElementTree.parse` on the template — all OK.
- [x] M37-S5-02 · Sandbox sanity for the SW routing regex
  (extend the M36-S5 27-case suite to include
  `offline_vocabulary` variants — should land in always-network
  passthrough alongside the other API routes).
- [x] M37-S5-03 · `docker exec odoo odoo --update language_learning
  --test-enable -u language_learning --stop-after-init --no-http`
  → existing 79 tests stay green.
- [x] M37-S5-04 · Live curl smoke against running Odoo —
  unauthenticated `GET /lexora_api/offline_vocabulary` → 303
  (auth redirect); authenticated → 200 with the expected
  `{status:'ok', words:[...], generated_at:N}` envelope.
- [x] M37-S5-05 · Browser smoke (user-side): reload extension /
  open `/my/practice/mobile` → M36 update banner appears
  (because VERSION bumped to v2) → tap Refresh → new SW takes
  control. Tap Dictionary tab → list renders alphabetically;
  search bar filters live as you type. Toggle back to Practice
  → swipe-card UX intact. Open in airplane mode → dictionary
  still works (data from IDB).

#### Blockers

(none yet — pure additive milestone on top of M36's foundations.)
...
**Scope:** Drag across YouTube subtitle words to trigger Quick Look on
multi-word phrases. Every downstream Quick Look feature (Add to
Vocabulary / Explain Grammar / 💡 Explain Slang/Idiom / 🎤 Practice
Pronunciation) inherits phrase support automatically because the card
already reads its word from internal data, not from re-reading the
caption DOM.

**The DOM-event strategy** (locked at planning — see ADR-034 once
Step S6 lands):

- **Native-first** with manual drag state machine as the documented
  fallback. Wins: browser draws the familiar blue highlight; selection
  naturally spans `.ytp-caption-segment` boundaries; no custom
  highlight CSS needed.
- **CSS override**: re-enable `user-select: text !important` on
  `.ytp-caption-window-container`, `.ytp-captions-container`,
  `.ytp-caption-segment`, and `.lx-sub-word`. YT applies `user-select:
  none` to the entire `.html5-video-container` subtree; our spans
  inherit it and the browser refuses to extend a range. Both
  `user-select` and the `-webkit-` prefix are written because YT
  serves different builds depending on UA.
- **Event firewall**: capture-phase `mousedown` / `mousemove` /
  `mouseup` listeners bound ONCE on the persistent
  `.ytp-caption-window-container` (NOT on each `.lx-sub-word` — those
  are replaced on every cue change). Each listener calls
  `e.stopPropagation()` only when the target is `closest('.lx-sub-word')`
  — and crucially does NOT call `preventDefault()` (which would kill
  native selection). YT's player listeners live on the bubble phase,
  so our capture-phase stop wins.
- **Phrase capture** in the `mouseup` post-microtask callback:
  `queueMicrotask` waits one tick so the browser finalises the
  selection range, then `window.getSelection().toString()` is fed
  through `_normalisePhrase` (trim → collapse whitespace → strip
  outer punctuation; internal apostrophes / hyphens preserved). If
  the result has ≥1 internal space, an `_lxSwallowNextClick` flag
  suppresses the about-to-fire per-span `click` and
  `_triggerPhraseOverlay(phrase)` runs through the existing M24
  Quick Look pipeline.
- **Click coexistence**: `_onWordClick` gains a one-line guard at the
  top that drains the swallow flag if set. Single-word selections
  (no internal space) skip the flag and the click handler takes over
  unchanged — zero M24 regression risk for the simple case.
- **Selection cleanup**: `window.getSelection().removeAllRanges()`
  fires after the overlay opens so the blue highlight doesn't linger
  while the user reads the card.

**Why these decisions:**

| Decision | Rationale |
|---|---|
| Native over manual drag | Browser owns cross-segment selection (e.g. captions split into two simultaneous lines); cursor I-beam is a familiar drag-affordance hint; zero custom highlight CSS. |
| Event firewall on the PERSISTENT container | `.ytp-caption-segment` and inner `.lx-sub-word` are recycled on every cue change. Binding on `.ytp-caption-window-container` survives those swaps; one listener trio for the whole page lifetime. |
| `stopPropagation` only, no `preventDefault` | `preventDefault` on `mousedown` would kill native selection. `stopPropagation` is sufficient — YT's player listeners are on the bubble phase. |
| `_lxSwallowNextClick` flag | The browser's `mouseup → click` contract is hard. The click handler still owns the simple case; the flag is set only for confirmed multi-word phrases. |
| Normalisation regex matches M2 / M27 / M34 | Lookup keys stay consistent across every extension feature that calls `lexora-define`. Internal apostrophes / hyphens preserved so `don't` and `mother-in-law` survive. |

**Fallback (Strategy B) — manual drag state machine on spans:**
`mousedown` on a span sets `_drag.anchor`; `mouseenter` on each
subsequent span appends to `_drag.spans`; `mouseup` on document
finalises and concatenates `_drag.spans.map(s => s.textContent).join(' ')`.
Custom `.lx-sub-selected` highlight CSS. Documented in PLAN.md M35
Strategy B; not wired in S1-S5; can be patched in a single commit if
browser smoke shows native selection breaks on a specific YT player
build (ambient mode / TV-mode shell / theatre mode).
...
- [ ] M35-S4-01 · Confirm Quick Look's three downstream buttons
  (Explain Grammar M28, Explain Slang/Idiom M32, Practice
  Pronunciation M33) work when the card's `_currentWord` is a
  multi-word phrase. All three buttons read from card-internal
  state, not from the caption DOM — so this should be a zero-effort
  inheritance.
- [ ] M35-S4-02 · The `/define` route already supports phrases
  (it dedupes against `language.entry.normalized_text` which can
  carry spaces, and `_live_translate` calls the translation service
  with the full string). Server-side: no change needed.

**Step M35-S5 — Edge-case browser smoke**

- [ ] M35-S5-01 · Drag-select across two simultaneous cue lines (top
  + bottom subtitle stack on dual-track videos). Expected: phrase
  spans both — browser's cross-text-node selection handles it.
- [ ] M35-S5-02 · Drag-select while a cue change happens mid-drag.
  Expected: selection lost (known limitation). Documented in ADR-034
  revisit triggers; user can re-drag.
- [ ] M35-S5-03 · Double-click a word. Expected: native single-word
  selection → `mouseup` sees no internal whitespace → click handler
  takes over → identical UX to plain click.
- [ ] M35-S5-04 · Triple-click a cue line. Expected: whole line
  selected → phrase overlay opens with full sentence (useful for
  grammar explanations).
- [ ] M35-S5-05 · Plain single-word click. Expected: zero
  behavioural change from M24.
- [ ] M35-S5-06 · YT-controls regression: click OUTSIDE the
  subtitle area (anywhere else on the player). Expected: video
  plays/pauses normally; event firewall is scoped to
  `closest('.lx-sub-word')` so non-caption clicks are untouched.
- [ ] M35-S5-07 · Drag-select WHILE the M34 radar is paused on a
  word (e.g. radar fired on "donkey", card is up, user drags
  another phrase in the same cue). Expected: works — radar
...
- [x] M24-15 · `portal_api.py` + `background.js` — added `/lexora_api/define_v2` as a second route alias on the same method to bypass Odoo bytecode cache; connectivity probe (`GET /health`) run on every define call to confirm translation-service reachability; background.js updated to call `define_v2` and explicitly log `live: true` flag. ✅
...
adding words from the browser. Extension provides a glassmorphism popup for manual
word capture without leaving the current tab.
...
## Completed Milestones (M18.5 – M21)

### M21 — Sentence Builder (Syntax Master)

**Status:** Complete and verified.
**Branch:** `m21_sentence_builder`

#### Sub-steps

- [x] M21-01 · `portal_sentence_builder.py` — `GET /my/sentence-builder` + `POST /my/sentence-builder/score`. ✅
- [x] M21-02 · `controllers/__init__.py` — import. ✅
- [x] M21-03 · `portal_sentence_builder.xml` — scramble tray, answer tray, Check button, XP badge. ✅
- [x] M21-04 · `website_menus.xml` — "Sentence Builder" (sequence=24). ✅
- [x] M21-05 · `__manifest__.py` — new files registered. ✅
- [x] M21-06 · `--update language_portal --stop-after-init` → 0 errors. ✅
- [x] M21-07 · `/my/sentence-builder` → 200; tiles, feedback, XP badge verified. ✅
- [x] M21-08 · Committed and pushed. ✅
...
**Status:** Complete and verified.
**Branch:** `m20_survival_phrasebook`

#### Sub-steps

- [x] M20-01 · `phrasebook_data.py` — 6 scenarios × ~15 phrases, EN/UK/EL. ✅
- [x] M20-02 · `portal_phrasebook.py` — `GET /phrasebook`, `GET /phrasebook/<scenario>`, `POST /phrasebook/copy-to-roleplay`. ✅
- [x] M20-03 · `controllers/__init__.py` — import. ✅
- [x] M20-04 · `portal_phrasebook.xml` — scenario grid + phrase list with EN/UK/EL tabs. ✅
- [x] M20-05 · `portal_roleplay.py` — `prefill` query param added. ✅
- [x] M20-06 · `website_menus.xml` — "Phrasebook" (sequence=27). ✅
- [x] M20-07 · `__manifest__.py` — new files. ✅
- [x] M20-08 · `--update language_portal --stop-after-init` → 0 errors. ✅
- [x] M20-09 · `/phrasebook` → 200; tabs, copy-to-roleplay verified. ✅
- [x] M20-10 · Committed and pushed. ✅
...
**Status:** Complete and verified.
**Branch:** `m19_idioms_hub`

#### Sub-steps

- [x] M19-01 · `language_idiom.py` — model definition. ✅
- [x] M19-02 · `models/__init__.py` — import. ✅
- [x] M19-03 · `ir.model.access.csv` — read-only for Language Users. ✅
- [x] M19-04 · `idioms_data.py` — 100+ entries seeded. ✅
- [x] M19-05 · Post-init hook to seed from `idioms_data.py`. ✅
- [x] M19-06 · `portal_idioms.py` — `GET /idioms`, `GET /idioms/<id>`, `POST /idioms/<id>/save`. ✅
- [x] M19-07 · `controllers/__init__.py` — import. ✅
- [x] M19-08 · `portal_idioms.xml` — card grid, flip animation, filters. ✅
- [x] M19-09 · `website_menus.xml` — "Idioms Hub" (sequence=26). ✅
- [x] M19-10 · `__manifest__.py` — new files. ✅
- [x] M19-11 · `--update language_portal --stop-after-init` → 0 errors. ✅
- [x] M19-12 · idiom count ≥100, `/idioms` → 200, save creates `language.entry`. ✅
- [x] M19-13 · Committed and pushed. ✅
...
**Status:** Complete and verified.
**Branch:** `m18.5_header_redesign`

#### Sub-steps

- [x] M18.5-01 · Read `docs/UI_REDESIGN_HEADER.md`. ✅
- [x] M18.5-02 · Audited existing `website.menu` records across all modules. ✅
- [x] M18.5-03 · Three parent menu records (Practice / Library / Tools) created. ✅
- [x] M18.5-04 · All child menu records re-parented to correct groups. ✅
- [x] M18.5-05 · Glassmorphism dropdown CSS added to `premium_ui.css`. ✅
- [x] M18.5-06 · Mobile collapse (Bootstrap toggler) verified. ✅
- [x] M18.5-07 · `--update language_portal,language_learning,language_pvp --stop-after-init`. ✅
- [x] M18.5-08 · All links resolve; no 404s. ✅
- [x] M18.5-09 · Committed and pushed. ✅
