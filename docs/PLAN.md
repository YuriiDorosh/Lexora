...
> Version: 1.4 (M26 AI Helpdesk — CPU-only RAG pipeline)
> Last updated: 2026-04-30
> Status: M0–M25 complete; M26 in progress
...
| M26 | AI Helpdesk — RAG Auto-Reply | 🔄 In progress | 100% automated helpdesk; `ai_mentor` FastAPI + pgvector + llama-cpp RAG; OdooBot chatter injection |
...
**Goal:** Every new helpdesk ticket receives an instant automated reply from OdooBot,
generated by a local RAG pipeline — no human agents, no cloud APIs, no GPU.
Authenticated users can view their full ticket history (with AI replies in the chatter)
at `/my/tickets`.

**Architecture:**

```
User submits ticket via /support (public form — Subject + Message only)
    ↓
lexora.ticket.create() override (lexora_helpdesk addon, self-contained)
    ↓  synchronous HTTP POST (timeout=35s)
ai_mentor FastAPI service  (port 8005)
    ├─ _embed(subject + description)  ← fastembed ONNX, CPU-only, 384-dim
    ├─ _retrieve()  ← pgvector cosine ANN, top-k=5, min_score=0.30
    └─ _generate()  ← llama-cpp-python Qwen2.5-1.5B Q4_K_M, CPU-only
    ↓  {"reply": "...", "rag_used": true, "sources": [...]}
Odoo: ticket.message_post(author=OdooBot, body=reply, email-silenced)
    ↓
Authenticated user views /my/tickets  →  /my/tickets/<id>  (chatter visible)
```

**API Contract (Odoo ↔ ai_mentor):**

| Endpoint | Method | Request | Response |
|---|---|---|---|
| `/health` | GET | — | `{status, llm_ready, embeddings_ready, pgvector_ok}` |
| `/answer` | POST | `{ticket_id, subject, description, language}` | `{reply, sources, rag_used}` |
| `/ingest` | POST | `{documents: [{content, metadata}]}` | `{indexed: N}` |

**Model quantization (CPU RAM budget on 8 GiB server):**

| Component | Model | Format | RAM |
|---|---|---|---|
| Embeddings | BAAI/bge-small-en-v1.5 | ONNX (fastembed) | ~100 MB |
| LLM generation | Qwen2.5-1.5B-Instruct | Q4_K_M GGUF | ~1.2 GiB |
| pgvector index | ivfflat cosine, lists=50 | Postgres shared_buffers | ~5 MB |

Upgrade path: set `AI_LLM_MODEL_REPO=microsoft/Phi-3-mini-4k-instruct-gguf` on hosts
with ≥12 GiB RAM. No code change required — env-configurable.

**pgvector setup:**

```sql
-- Activated in ai_mentor init code on first start:
...
CREATE INDEX IF NOT EXISTS ai_mentor_docs_emb_idx
    ON ai_mentor_docs USING ivfflat (embedding vector_cosine_ops)
    WITH (lists = 50);
```

Requires the Postgres image to be `pgvector/pgvector:pg15` (drop-in for `postgres:15`).
Existing `postgres_data_odoo` volume is fully compatible — no data migration needed.

**Work:**

1. `docker_compose/db/docker-compose.yml` — change `image: postgres:15` →
   `pgvector/pgvector:pg15` (and backup service). ✅
2. `services/ai_mentor/` — full FastAPI RAG service:
   - `requirements.txt` — `llama-cpp-python`, `fastembed`, `psycopg2-binary`,
     `pgvector`, `huggingface-hub`, `requests`. ✅
   - `main.py` — pgvector init + fastembed embeddings + GGUF LLM loading +
     `_retrieve()` + `_generate()` + seed docs + `/health` + `/answer` + `/ingest`. ✅
3. `docker_compose/ai_mentor/Dockerfile` — `python:3.11-slim` + cmake/build-essential
   (for llama-cpp-python wheel fallback). ✅
4. `docker_compose/ai_mentor/docker-compose.yml` — port 8005, `ai_mentor_models`
   volume, all env vars. ✅
5. `src/addons/lexora_helpdesk/` — self-contained Odoo addon (no third-party deps):
   - `__manifest__.py` — depends on `website`, `mail`, `portal`. ✅
   - `models/lexora_ticket.py` — `lexora.ticket` model; inherits `mail.thread` +
     `mail.activity.mixin`; `create()` override calls `/answer` synchronously and
     injects OdooBot reply via `message_post()` with email-silencing context vars. ✅
   - `controllers/portal.py` — `GET/POST /support` (public, two-field form);
     `CustomerPortal` subclass for `/my/tickets` list + `/my/tickets/<id>` detail. ✅
   - `views/portal_support.xml` — dark gradient hero, Bootstrap card, CSRF token. ✅
   - `views/portal_tickets.xml` — portal home widget, ticket list table with pager,
     ticket detail with chatter (OdooBot reply visible). ✅
   - `views/lexora_ticket_views.xml` — backend list/form/search + Helpdesk menu. ✅
   - `security/ir.model.access.csv` — user/admin/portal access rows. ✅
   - `security/record_rules.xml` — users see only own tickets; admins see all. ✅
   - `data/ir_config_parameter.xml` — seeds `lexora.ai_mentor.{url,timeout,enabled}`. ✅
   - `data/website_menu.xml` — "Support" navbar entry → `/support`, sequence 90. ✅
6. `Makefile` — `AI_MENTOR_CONTAINER`, `up-ai-mentor`, `down-ai-mentor`,
   `up-ai-mentor-no-cache`, `logs-ai-mentor`; added to `up-dev` / `down-dev` /
   `ps-dev` / `logs-dev`. ✅
7. `env.example` — `AI_LLM_*` and `AI_EMBED_*` vars documented. ✅

**Verification:**

```bash
# 1. Rebuild Postgres with pgvector (first time only — data volume unchanged)
make down-db && make up-db
docker exec postgres psql -U odoo -d lexora -c "CREATE EXTENSION IF NOT EXISTS vector;"
# → CREATE EXTENSION

# 2. Build and start ai_mentor service
make up-ai-mentor-no-cache
# First start downloads ~1.1 GiB GGUF model (allows ~2–5 min)

curl http://localhost:8005/health
# → {"status":"ok","llm_ready":false,"embeddings_ready":false,"pgvector_ok":true}
# Wait for model load; retry until llm_ready=true (~30–120s after download)

# 3. Direct answer test
curl -X POST http://localhost:8005/answer \
  -H "Content-Type: application/json" \
  -d '{"ticket_id":1,"subject":"How do I add words?","description":"I want to save new vocabulary.","language":"en"}'
# → {"reply":"...","rag_used":true,"sources":[...]}

# 4. Install lexora_helpdesk Odoo addon
docker exec odoo odoo --config /etc/odoo/odoo.conf \
  -d lexora --init lexora_helpdesk --stop-after-init

# 5. Submit a ticket via /support → confirm OdooBot reply in chatter within ~35s
# 6. Log in and visit /my/tickets → ticket list visible
#    Click ticket → /my/tickets/<id> → chatter with AI reply rendered
```

**Fallback behaviour:** If the LLM has not loaded yet (still downloading/initialising),
`_generate()` returns `_stub_reply()` — a polite acknowledgement — so users never see
an error. The Odoo-side timeout is 35 s; if the service is unreachable, the stub reply
is also used (`lexora_helpdesk/models/lexora_ticket.py` catches all exceptions).

**Email silencing:** OdooBot replies are posted with context vars
`mail_post_autofollow=False`, `mail_create_nosubscribe=True`, `mail_notify_force_send=False`
to prevent Gmail/email notifications on every ticket.

**Zero human agents:** The system parameter `lexora.ai_mentor.enabled` defaults to `1`.
Set it to `0` in `ir.config_parameter` to pause auto-reply without uninstalling the addon.
...
M0 → M1 → M2 → M3
          ...
          M9 → M10 → M11 → M12 → M13 → M14 → M15 → M16 → M17 → M18
                                          ...
                                          (Contextual)   (Subtitles)  (New Tab)
                                                                    ↓
                                                    M26 (AI Helpdesk — RAG, independent)
```

M26 is independent of the browser extension track (M22–M25). It requires only:
- A running Postgres with pgvector (M0 infrastructure)
- The `ai_mentor` Docker service reachable from the Odoo container network
- No third-party helpdesk addons — `lexora_helpdesk` is fully self-contained
