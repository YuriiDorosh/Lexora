...
```
User submits ticket (portal or email)
    ↓
helpdesk.ticket.create() override (language_helpdesk addon)
...
Odoo: ticket.message_post(author=OdooBot, body=reply)
```
...
4. `docker_compose/ai_mentor/docker-compose.yml` — port 8005, `ai_mentor_models`
   volume, all env vars. ✅
5. `src/addons/language_helpdesk/` — new Odoo addon:
   - `__manifest__.py` — depends on `helpdesk`, `mail`. ✅
   - `models/helpdesk_ai.py` — `_inherit = 'helpdesk.ticket'`, overrides `create()`
     to call `/answer` synchronously and inject reply via `message_post()`. ✅
   - `data/ir_config_parameter.xml` — seeds `language.ai_mentor.url`,
     `language.ai_mentor.timeout`, `language.ai_mentor.enabled`. ✅
...
# 4. Install language_helpdesk Odoo addon
docker exec odoo odoo --config /etc/odoo/odoo.conf \
  -d lexora --init language_helpdesk --stop-after-init

# 5. Create a helpdesk ticket via portal and confirm OdooBot reply appears in chatter
# → message_post() with author_id=base.partner_root within ~35s
```
...
is also used (`language_helpdesk/models/helpdesk_ai.py` catches all exceptions).

**Zero human agents:** The system parameter `language.ai_mentor.enabled` defaults to `1`.
...
- The `helpdesk` Odoo module installed
